...
def login():
    ...
    return render_template('auth/signin.html', title=_('Sign In'), form=form)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if trade is None:
            trade = Trade(trader=current_user)
            db.session.add(trade)
            db.session.commit()
        return redirect(url_for('main.index'))
    return render_template('auth/signup.html', title=_('Register'),
                           form=form)
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        flash(
            _('Check your email for the instructions to reset your password'), category="info")
        return redirect(url_for('auth.login'))
